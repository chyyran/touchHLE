...
unsafe fn present_renderbuffer(gles: &mut dyn GLES, window: &mut Window) {
    ...
    // by adjusting both texture coordinates, and where in the POT texture it ends up
    // so we don't sample from uninitialized pixels.
    let (xoffset, yoffset) = match window.current_rotation() {
        ...
        // is rotated to the bottom left corner, so we need to shift the texture to the
        // top left corner of the POT target.
        ...
        // so we need to shift the textyre to the bottom right
    ...
    //
    // let offset = ((texture_height.abs_diff(render_height as u32) as f32 / render_height as f32));
    // eprintln!("{offset}");
    // Draw the quad
    present_frame(
        gles,
